package sg.edu.rp.c346.s19024292.p09_gettingmylocationsenhanced;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        File folder_I = new File(folderLocation_I); if (!folder_I.exists()){
            boolean result = folder_I.mkdir(); if (result){
                ...
                mLocationCallback = new LocationCallback() {
                    ...
                    public void onLocationResult(LocationResult locationResult) {
                        if (locationResult != null) {
                            ...
                            LatLng updated = new LatLng(data.getLatitude(), data.getLongitude());
                            if (central == null) {
                                ...
                                central.setPosition(updated);
                                try {
                                    String folderLocation_I = getFilesDir().getAbsolutePath() + "/Folder";
                                    File targetFile_I = new File(folderLocation_I, "location.txt");
                                    FileWriter writer_I = new FileWriter(targetFile_I, true); writer_I.write(updated.latitude+","+updated.longitude + "\n");
                                    writer_I.flush();
                                    writer_I.close();
                                } catch (Exception e) {
